...
def test_add_new_pet_with_valid_data(name='Мышка', animal_type='крыса',
                                     age='1', pet_photo='images/IMG_20200601_200141.jpg'):
...
def test_successful_delete_self_pet():
    ...
    if len(my_pets['pets']) == 0:
        pf.add_new_pet(auth_key, "Тайсон", "Кот", "2", "images/animal-cute.jpg")
        _, my_pets = pf.get_list_of_pets(auth_key, "my_pets")
...
def test_successful_update_self_pet_info(name='Николка', animal_type='Собак', age=3):
...
def test_update_another_pet_info(name='pesik_new', animal_type='dog', age='10'):
    ...
    # Проверяем - если список второго пользователя питомцев пустой, то добавляем нового
    if len(second_user_pets['pets']) == 0:
        pf.add_new_pet(auth_key2, "аистенок", "аист", "2", "images/IMG_20200601_200141.jpg")
...
def test_update_with_incorrect_data(name=' ', animal_type=' ', age=' '):
    ...
    if len(my_pets['pets']) == 0:
        pf.add_new_pet(auth_key, "аист", "ковбой", "2", "images/IMG_20200601_200141.jpg")
...
#8
def test_add_new_pet_with_invalid_age(name='белка', animal_type='белченок',
                                     age='dvenadcat', pet_photo='images/animal-cute.jpg'):
    """Проверяем что можно добавить питомца с некорректным возрастом, указанным кириллицей
     (!) Тест заканчивается со статусом FAIL т.к. он отлавливает баг"""
...
#9
def test_add_new_pet_with_invalid_photo_type(name='Ктото', animal_type='мой',
                                     age='50', pet_photo='images/284932459.gif'):
...
#10
def test_positive_scenario(name='kisa', animal_type='белка', age='1', pet_photo='images/animal-cute.jpg'):
    """Проверяем сценарий: создаем нового питомца - обновляем данные по нему и удаляем питомца"""
    ...
    status_update, update_pet = pf.update_pet_info(auth_key, new_pet_id, 'Никола', 'песель', 2)
